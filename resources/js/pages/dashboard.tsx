...
const InvoiceUploadDropzone = () => {
    ...
    useEffect(() => {
        if (currentUploadIndex === null && files.length > 0) {
            const remainingFiles = files.filter((file) => fileStatus[file.name]?.status !== 'success');
    ...
    const getFileIcon = (file: File) => {
        if (file.type === 'application/pdf') {
            return (
                <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" className="size-8" viewBox="0 0 256 256">
                    <path
                        fill="currentColor"
    ...
    return (
        <div
            {...getRootProps()}
            className={`dark:border-sidebar-border relative aspect-video overflow-hidden rounded-xl border border-transparent transition-all duration-200 ${isDragActive ? 'border-blue-300 bg-blue-50/30 dark:border-blue-500/50 dark:bg-blue-900/10' : 'border-dashed hover:border-gray-300 hover:bg-gray-50 dark:hover:border-gray-500/50 dark:hover:bg-gray-800/30'} `}
        >
            <input {...getInputProps()} />
            ...
            <div className="flex h-full flex-col p-4">
                {files.length > 0 ? (
                    <div className="flex h-full flex-col">
                        <div className="mb-2 flex items-center justify-between">
                            ...
                            <div className="flex gap-2">
                                <button
                                    ...
                                    className="rounded bg-red-50 px-2 py-1 text-xs text-red-600 hover:bg-red-100 disabled:opacity-50 dark:bg-red-900/20 dark:text-red-400"
                                >
                                    Clear All
                                </button>
                                <button
                        ...
                        <div className="flex-1 overflow-hidden">
                            <ScrollArea className="h-full max-h-[calc(100%-5px)]" ref={scrollAreaRef}>
                                <div className="pr-3ZZZ grid gap-2">
                        ...
                        <div className="mt-2 text-center">
                            <p className="text-xs text-gray-500">Drop more files or click to browse</p>
                        </div>
                    </div>
                ) : (
                    <div className="flex h-full flex-col items-center justify-center text-center">
                        ...
                        <p className="mb-1 font-medium text-gray-700 dark:text-gray-300">{isDragActive ? 'Drop invoices here' : 'Upload Invoices'}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400">Drag & drop invoice files, or click to browse</p>
                        <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">JPG, PNG or PDF (max. 5MB each)</p>
                    </div>
                )}
            </div>
        </div>
